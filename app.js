...
function gameScreen() { if (!state.game) return `<main class="page"><div class="empty">Loading the table…</div></main>`; const bySeat = [0,1,2,3].map((seat) => state.players.find((player) => player.seat === seat)).filter(Boolean); const me = state.players.find((p) => p.user_id === mine()); const mySeat = me ? me.seat : 0;
      ...
      const positions = ['left', 'top', 'right']; const opponentHtml = others.map((player, i) => `<div class="opponent ${positions[i] || 'top'} ${state.game.current_player_key === player.player_key ? 'turn-ring' : ''}">${avatar(player.display_name, player.avatar_url)}<div class="opponent-info"><div class="op-name">${escapeHtml(player.display_name)}</div><div class="card-count">${player.hand_count} cards${player.passed ? ' · passed' : ''}</div></div></div>`).join(''); const pile = (state.game.pile || []).map(cardFace).join(''); const myTurn = state.game.status === 'playing' && state.game.current_player_key === mine(); const cards = state.hand.map((card) => `<button class="card ${redSuits.has(card.suit) ? 'red' : ''} ${state.selected.includes(card.id) ? 'selected' : ''}" data-action="toggle-card" data-card="${card.id}" ${myTurn ? '' : 'disabled'}>${card.rank}<span>${suitSymbol[card.suit]}</span></button>`).join(''); const pileNote = state.game.last_play ? `${escapeHtml(state.game.last_play.player_name)} played ${state.game.last_play.label}` : 'New trick — any legal combination leads'; const result = state.game.status === 'finished' ? roundCompleteModal(bySeat) : ''; return `<main class="table"><div class="table-top"><button class="btn" data-action="back-lobby">← Leave table</button><div class="game-title"><span class="status-dot"></span>${escapeHtml(state.room?.name || 'Tien Len')} · ${state.game.status === 'playing' ? 'In play' : state.game.status}</div><button class="btn icon-btn" data-action="refresh-game" aria-label="Refresh game">↻</button></div><section class="table-felt">${opponentHtml}<div class="center-play"><div class="pile">${pile || '<span class="subtle">The cards are waiting</span>'}</div><div class="pile-note">${pileNote}</div>${state.game.last_play?.passed_players?.length ? `<div class="pass-note">${state.game.last_play.passed_players.length} passed</div>` : ''}</div></section><section class="hand-zone"><div class="hand">${cards}</div></section><div class="game-controls">${myTurn ? `<button class="btn" data-action="pass" ${state.game.last_play ? '' : 'disabled'}>Pass</button><button class="btn btn-primary" data-action="play" ${state.selected.length ? '' : 'disabled'}>Play ${state.selected.length ? `(${state.selected.length})` : ''}</button>` : `<span class="hint">${state.game.status === 'finished' ? 'Round complete' : `Waiting for ${escapeHtml(state.players.find((p) => p.player_key === state.game.current_player_key)?.display_name || 'the next player')}…`}</span>`}</div>${result}</main>`; }
const ordinal = (i) => ['1st', '2nd', '3rd', '4th'][i] || `${i + 1}th`;
// Shown when a hand finishes: who placed where and how many points they earned this round,
// plus the running match scoreboard. The host can deal straight into the next round (or a
// fresh match once round 4 is done) without leaving the room, keeping the same seated players.
function roundCompleteModal(bySeat) {
  const finishOrder = state.game.finish_order || [];
  const placements = finishOrder.map((key, i) => { const p = state.players.find((pl) => pl.player_key === key); const pts = state.game.round_points?.[String(p?.seat)]; return `<li><b>${ordinal(i)}</b> ${escapeHtml(p?.display_name || 'Player')}${pts != null ? ` <span class="subtle">+${pts} pt${pts === 1 ? '' : 's'}</span>` : ''}</li>`; }).join('');
  const scores = bySeat.map((p) => ({ name: p.display_name, score: state.room?.match_scores?.[String(p.seat)] || 0, seat: p.seat })).sort((a, b) => b.score - a.score);
  const scoreboard = scores.map((s) => `<li>${escapeHtml(s.name)}<b>${s.score}</b></li>`).join('');
  const matchComplete = state.room?.status === 'match_complete';
  const champion = matchComplete ? bySeat.find((p) => p.seat === state.room?.match_winner_seat) : null;
  const roundLabel = matchComplete ? `Match complete · ${scores.length ? 4 : ''} rounds played` : `Round ${state.game.round_number || 1} of 4 complete`;
  const nextAction = isOwner() ? `<button class="btn btn-primary" data-action="start-game">${matchComplete ? 'Start a new match' : `Deal round ${state.room?.round_number || 2}`}</button>` : `<span class="hint">Waiting for the host to ${matchComplete ? 'start a new match' : 'deal the next round'}…</span>`;
  return `<div class="modal-backdrop"><section class="modal panel round-summary"><p class="eyebrow">${roundLabel}</p><h2>${matchComplete ? `${escapeHtml(champion?.display_name || 'A player')} wins the match!` : `${escapeHtml(state.game.winner_name || 'A player')} goes out first`}</h2><div class="round-summary-grid"><div><h3>This round</h3><ol class="placement-list">${placements}</ol></div><div><h3>Match score</h3><ul class="scoreboard-list">${scoreboard}</ul></div></div><div class="btn-row"><button class="btn" data-action="back-lobby">Back to foyer</button>${nextAction}</div></section></div>`;
}
...
async function loadRoom(id = state.room?.id) {
  ...
  const [{ data: room, error }, { data: members, error: memberError }] = await Promise.all([
    supabase.from('rooms').select('id, code, name, visibility, owner_id, status, round_number, match_scores, match_winner_seat').eq('id', id).single(),
