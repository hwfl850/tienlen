...
async function bootstrap() { if (!configured) { render(); return; } if ('serviceWorker' in navigator) navigator.serviceWorker.register('./sw.js').catch(() => {}); const { data: { session } } = await supabase.auth.getSession(); state.session = session; if (session) { try { await loadProfile(); await goHome(); } catch (error) { console.error(error); toast('Could not load your profile.', 'error'); render(); } } else render(); supabase.auth.onAuthStateChange(async (_event, sessionNow) => { state.session = sessionNow; if (!sessionNow) { clearChannel(); state.profile = null; state.avatarUrl = null; state.view = 'auth'; render(); return; } try { await loadProfile(); await goHome(); } catch (error) { console.error(error); } }); }
bootstrap();
